...
lines <- c(
  ...
  "    df <- scored_r()",
  "    if (is.null(df)) return(NULL)",
  "    df[!is.na(df$has_european_site) & df$has_european_site == TRUE, ]",
  "  })",
  "",
  "  reg_r <- reactive({",
  "    df <- dat$reg",
  ...
  "      pct(mean(df$flag_no_health_literacy, na.rm=TRUE)) else \"N/A\"",
  ...
  "    rates <- sapply(avail$col, function(cc) mean(df[[cc]], na.rm=TRUE) * 100)",
  ...
  "      pct(mean(df$flag_no_diversity_target, na.rm=TRUE)) else \"N/A\"",
  "    kdf <- data.frame(",
